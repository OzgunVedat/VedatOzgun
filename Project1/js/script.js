...
$("#selCountry").on("change", function() {
	...
	document.getElementById('countryIdEasyButton').innerHTML = countryOptionText;
    $.ajax({
        url: "./php/getCountryNames.php",
        type: "POST",
        dataType: 'json',
        success: function(result) {
            ...
            for (let i = 0; i < result.data.border.features.length; i++) {
                if (result.data.border.features[i].properties.name === countryOptionText) {
					document.getElementById('countryCodeIdEasyButton').innerHTML = result.data.border.features[i].properties.iso_a2;
